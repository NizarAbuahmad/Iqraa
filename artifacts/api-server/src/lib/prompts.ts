...
const LESSON_STYLE_RULES_AR: Record<string, string> = {
  direct: `أسلوب مباشر (أنا أفعل / نحن نفعل / أنت تفعل). التزم بالتسلسل: شرح بأمثلة محلولة كاملة، ثم حل مشترك يوجّهه الطلبة خطوة بخطوة، ثم عمل فردي صامت.
...
- "mainActivity": اطرح سؤال التحقيق دون إعطاء القاعدة، ويستكشف الطلبة أمثلة أو بيانات ويكتبون **تخمينهم** للقاعدة.
...
ممنوع تمامًا: أي عبارة تمنع النقاش بين الطلبة («المناقشة بين الطلبة مؤجّلة») — فهي تناقض الأسلوب الذي بُنيت عليه الحصة.`,
};
...
ممنوع تمامًا: المجموعات، توزيع الأدوار، العروض التقديمية، أي عبارة فيها «قسّم الطلبة» أو «كل مجموعة». حجم المجموعة هو «فردي».
...
- اكتب خطوات يستطيع المعلم تنفيذها كما هي: ماذا يقول، ماذا يوزّع، كم دقيقة، وماذا يفعل الطلبة.
...
أعد JSON بالشكل الآتي (بالعربية):
{
  ...
  "differentiation": "كيف تتعامل مع الطلبة ذوي المستويات المختلفة",

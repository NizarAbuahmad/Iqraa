 ...
 * teacher can land on a pair no book covers — الصف العاشر + التاريخ، where
 * history joined MVP_SUBJECT_IDS on 2026-09-09 as a brand-new subject with
 ...
 * financial-literacy, then geography (retired the same week its Grade 10 book
 * arrived — see `subjectGradeCoverage.test.ts`'s `KNOWN_BOOKLESS`). That churn
 * is the allowlist doing its job — a pair gaining a book is meant to fail a
 * test and make someone look, and each subject losing this slot the day its
 * Grade 10 book arrives is expected, not a regression.
 *
...
describe('picker scope — grade/subject pairs with no book', () => {
  ...
  it('hides history for grade-10 but not for grade-9', () => {
    const idx = getPickerSubjects().findIndex(s => s.id === 'history');
    assert.ok(idx >= 0, 'history is expected in the MVP picker list');
  ...
  it('hiding never shifts the index an option reports', () => {
    ...
    //
    // As of 2026-09-09 the real bookless mask lives on grade-10 (history is
    // one of three gaps there — geography closed the same week) rather than
    // grade-9. The synthetic mask stays regardless — it pins the property
    // directly instead of depending on the catalogue continuing to supply an
    // interesting shape.
...
describe('picker scope — the generate-time backstop', () => {
  ...
  it('names the offending pair so the message can say which', () => {
    const scope = scopeWithoutCurriculum('grade-10', 'history', 'ar');
    assert.ok(scope, 'grade-10 + history must be refused');
    assert.equal(scope.subject, 'التاريخ');
    assert.equal(scope.grade, 'الصف العاشر');
  });

  it('answers in the display language', () => {
    const scope = scopeWithoutCurriculum('grade-10', 'history', 'en');
    assert.ok(scope);
    assert.equal(scope.subject, 'History');
  });

  it('passes a pair that has a book', () => {
    assert.equal(scopeWithoutCurriculum('grade-9', 'history', 'ar'), null);

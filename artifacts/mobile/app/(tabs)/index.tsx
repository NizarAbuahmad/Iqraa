...
/** Safe when a non-tab host omits the BottomTabBarHeight context. */
...
export default function ClassesScreen() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.background }}>
      {/* No back arrow: this is the tab the app opens on, so there is
          nothing behind it to pop. */}
      ...
      {loading ? (
        ...
        <FlatList
          data={classes}
          keyExtractor={c => c.id}
          contentContainerStyle={{ padding: 20, paddingBottom: tabBarHeight + 100, gap: 12 }}
          ...
          ListEmptyComponent={
            error ? null : (
              <View style={styles.empty}>
                ...
                {/* The empty state carries the action. A floating "+" is a
                    weak affordance for the only thing there is to do here. */}
                <Pressable
                  onPress={() => setShowNew(true)}
                  style={[styles.emptyCta, { backgroundColor: ACCENT }]}
                >
                  <Ionicons name="add" size={18} color="#fff" />
                  <Text style={[styles.emptyCtaText, { fontFamily: 'Cairo_600SemiBold' }]}>
                    {t('createClass')}
                  </Text>
                </Pressable>
              </View>
      ...
      {/* Only once there is a list. On the empty screen the CTA above is
          the same action, and two of them read as two different things. */}
      {classes.length > 0 && (
      <Pressable
        onPress={() => setShowNew(true)}
        style={[
          styles.fab,
          // Clear the tab bar — this screen used to be pushed on a stack,
          // where insets.bottom alone was enough.
          { backgroundColor: ACCENT, bottom: (tabBarHeight || insets.bottom) + 24 },
        ]}
      >
...
const styles = StyleSheet.create({
  ...
  emptyCtaText: { color: '#fff', fontSize: 14 },

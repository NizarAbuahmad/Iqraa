        ...
        <View style={styles.headerRow}>
          ...
          {unreadCount > 0 && (
            <Pressable onPress={markAllRead} style={[styles.markAllBtn, { backgroundColor: colors.secondary, borderRadius: 20 }]}>
              <Text style={[styles.markAllText, { color: colors.primary, fontFamily: 'Inter_500Medium' }]}>Mark all read</Text>
            </Pressable>
          )}
        </View>
      </View>
        ...
        renderItem={({ item: n }) => {
          ...
          return (
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                <Text style={[styles.notifTitle, { color: colors.foreground, fontFamily: n.read ? 'Inter_500Medium' : 'Inter_600SemiBold' }]}>
                  {n.title}
                </Text>
                <Text style={[styles.notifBody, { color: colors.mutedForeground, fontFamily: 'Inter_400Regular' }]} numberOfLines={2}>
                  {n.body}
                </Text>
                <Text style={[styles.notifTime, { color: colors.mutedForeground, fontFamily: 'Inter_400Regular' }]}>{n.time}</Text>
              </View>
...
const styles = StyleSheet.create({
  ...
  notifCard: { flexDirection: 'row', padding: 14, gap: 12, borderWidth: 1, alignItems: 'flex-start' },

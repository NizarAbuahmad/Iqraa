      ...
      >
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}>
          <SettingRow
            ...
            right={
              ...
              >
                <Text style={[{ color: lang === 'ar' ? colors.primaryForeground : colors.mutedForeground, fontFamily: 'Inter_500Medium', fontSize: 12 }]}>
        ...
        <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border, borderRadius: colors.radius }]}>
          <SettingRow
            ...
            isRTL={isRTL}
            colors={colors}
            right={<Text style={[{ color: colors.mutedForeground, fontFamily: 'Inter_400Regular', fontSize: 13 }]}>1.0.0</Text>}
          />
          <View style={[styles.divider, { backgroundColor: colors.border }]} />
...
function SectionLabel({ label, isRTL, colors, top }: { label: string; isRTL: boolean; colors: ReturnType<typeof useColors>; top?: boolean }) {
  return (
    <Text style={[styles.sectionLabel, { color: colors.mutedForeground, fontFamily: 'Inter_500Medium', marginTop: top ? 20 : 0, textAlign: isRTL ? 'right' : 'left' }]}>
      {label}
    </Text>
  );
}
...
function SettingRow({ icon, label, colors, isRTL, right, onPress }: {
  ...
  const inner = (
    <View style={[styles.settingRow, { flexDirection: isRTL ? 'row-reverse' : 'row' }]}>
      ...
      <Text style={[styles.settingLabel, { color: colors.foreground, fontFamily: 'Inter_500Medium', flex: 1, textAlign: isRTL ? 'right' : 'left' }]}>
        {label}
      </Text>

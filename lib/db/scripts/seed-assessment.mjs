...
/**
 * Where the connection string comes from, and why it depends on the machine.
 *
 * Locally the repo-root `.env` wins outright — the `delete` below is
 * deliberate, so a `DATABASE_URL` left in a shell cannot quietly point a seed
 * at a database the developer did not mean. On a server there is no `.env`;
 * the environment **is** the configuration, and forcing a missing file there
 * is what kept this script from ever running on deploy.
 */
...
if (!process.env.DATABASE_URL) {
  console.error(
    "DATABASE_URL is not set. Add it to the repo-root .env locally, or set it in the environment on a server.",
  );
  process.exit(1);
}

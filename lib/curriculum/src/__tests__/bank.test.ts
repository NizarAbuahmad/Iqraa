...
describe('manifest shape', () => {
  ...
  it('never puts another subject\'s tag on a source', () => {
    ...
    const prefixFor: Record<CurriculumSource['subject'], RegExp> = {
      ...
      // Grade 4 joined 2026-09-15, so this widens from `g9-` to `g\d+-` the
      // way the others already did. Slug is `eng`, not this record's `english`.
      english: /^(eng-s[12]|g\d+-eng-s[12])$/,
      geography: /^geo-s[12]$/,
      // Grade 4 joined 2026-09-15. Slug is `digital`, not the full key.

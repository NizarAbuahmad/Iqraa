/**
 * Grade 10 Digital Skills (المهارات الرقمية) — Semester 1 (NCCD student book).
 * Source of truth: data/iqra_curriculum_g10_digital_sem1.json
 *
...
import raw from '../data/iqra_curriculum_g10_digital_sem1.json' with { type: 'json' };
import {
  lessonKbId,
  lessonKbPrefix,
  objectiveId,
...
export const DIGITAL_S1_BOOK_ID = 'kb-digital-10-s1';

export type DigitalSem1Vocab = { ar: string; en: string };

export type DigitalSem1Lesson = {
  id: string;
  order: number;
  type?: string;
  title_ar: string;
  ...
  periods: number | null;
  objectives: string[];
  vocabulary: DigitalSem1Vocab[];
};

export type DigitalSem1Unit = {
  id: string;
  number: number;
  title_ar: string;
  title_en: string;
  ...
  lessons: DigitalSem1Lesson[];
};

export type DigitalSem1CurriculumFile = {
  meta: Record<string, unknown>;
  units: DigitalSem1Unit[];
};

export const nccdG10DigitalSem1 = raw as DigitalSem1CurriculumFile;
...
const SCOPE: CurriculumIdScope = { gradeId: 'grade-10', subject: 'digital', semester: 1 };

/** Stable KB unit id (e.g. u1 → kbu-digital-s1-nccd-u1). */
export function digitalSem1UnitKbId(jsonUnitId: string): string {
  return unitKbId(SCOPE, jsonUnitId);
}

/** Stable KB lesson id (e.g. u1_l1 → kbl-digital-s1-nccd-u1_l1). */
export function digitalSem1LessonKbId(jsonLessonId: string): string {
  return lessonKbId(SCOPE, jsonLessonId);
}

/** Look up a JSON lesson by mapped KB lesson id. */
export function findDigitalSem1LessonByKbId(kbLessonId: string): DigitalSem1Lesson | null {
  const prefix = lessonKbPrefix(SCOPE);
  if (!kbLessonId.startsWith(prefix)) return null;
  const jsonId = kbLessonId.slice(prefix.length);
  for (const u of nccdG10DigitalSem1.units) {
...
/**
 * Map the digital-skills JSON into KB units + lessons for kb-digital-10-s1.
 * Arabic strings are copied byte-for-byte from the JSON (which copies the
 * student book's printed table of contents).
 */
export function buildDigitalSem1Catalog(): { units: DigitalKbUnit[]; lessons: DigitalKbLesson[] } {
  const units: DigitalKbUnit[] = nccdG10DigitalSem1.units.map(u => ({
    id: digitalSem1UnitKbId(u.id),
    bookId: DIGITAL_S1_BOOK_ID,
    order: u.number,
    titleAr: u.title_ar,
    titleEn: u.title_en,
  }));
  ...
  for (const u of nccdG10DigitalSem1.units) {
    const uKbId = digitalSem1UnitKbId(u.id);
    for (const lesson of u.lessons) {
      const vocab = lesson.vocabulary ?? [];
      const objectives = lesson.objectives ?? [];
      lessons.push({
        id: digitalSem1LessonKbId(lesson.id),
        unitId: uKbId,
        order: lesson.order,
...
export const DIGITAL_S1_CURRICULUM_BOOK_ID = 'book-digital-10-s1';
...
/**
 * Map the digital-skills S1 JSON into curriculum-browser Unit/Lesson rows.
 *
 ...
 */
export function buildDigitalSem1BrowserCatalog(): {
  units: DigitalBrowserUnit[];
  lessons: DigitalBrowserLesson[];
} {
  const units: DigitalBrowserUnit[] = nccdG10DigitalSem1.units.map(u => {
    const lessonTitles = u.lessons.map(l => l.title_ar).join(' · ');
    return {
      id: digitalSem1UnitKbId(u.id),
      bookId: DIGITAL_S1_CURRICULUM_BOOK_ID,
      name: u.title_en,
      nameAr: u.title_ar,
  ...
  for (const u of nccdG10DigitalSem1.units) {
    ...
    for (const lesson of u.lessons) {
      ...
      const lKbId = digitalSem1LessonKbId(lesson.id);
      lessons.push({
        id: lKbId,
        unitId: uKbId,

/**
 * Grade 6 Social Studies (الدراسات الاجتماعية) — Semester 1 (NCCD student book).
 * Source of truth: data/iqra_curriculum_g6_social_sem1.json
 *
...
import raw from '../data/iqra_curriculum_g6_social_sem1.json' with { type: 'json' };
import { lessonKbPrefix, unitKbPrefix, type CurriculumIdScope } from '../curriculumIds.ts';
import { makeNccdCatalog, type NccdCurriculumFile, type NccdLesson } from './nccdCatalog.ts';

/** Book id in KB_BOOKS that this JSON fills. */
export const G6_SOCIAL_S1_BOOK_ID = 'kb-social-6-s1';

/** Book id in curriculumData.BOOKS this catalog fills (browser id space). */
export const G6_SOCIAL_S1_CURRICULUM_BOOK_ID = 'book-social-6-s1';

const SCOPE: CurriculumIdScope = { gradeId: 'grade-6', subject: 'social', semester: 1 };

const catalog = makeNccdCatalog({
  scope: SCOPE,
  kbBookId: G6_SOCIAL_S1_BOOK_ID,
  browserBookId: G6_SOCIAL_S1_CURRICULUM_BOOK_ID,
  raw,
});

export const nccdG6SocialSem1: NccdCurriculumFile = catalog.curriculum;

/** Stable KB unit id (e.g. u1 → kbu-g6-social-s1-nccd-u1). */
export const g6SocialSem1UnitKbId = catalog.unitKbId;

/** Stable KB lesson id (e.g. u1_l1 → kbl-g6-social-s1-nccd-u1_l1). */
export const g6SocialSem1LessonKbId = catalog.lessonKbId;

/** Look up a JSON lesson by mapped KB lesson id. */
export const findG6SocialSem1LessonByKbId: (kbLessonId: string) => NccdLesson | null =
  catalog.findLessonByKbId;

/** True for every unit in this book — see the header for why it is unconditional. */
export function isG6SocialSem1TitleOnlyUnit(unitKbId: string): boolean {
...
export function isG6SocialSem1TitleOnlyLesson(kbLessonId: string): boolean {
  return kbLessonId.startsWith(lessonKbPrefix(SCOPE));
}

export const buildG6SocialSem1Catalog = catalog.buildCatalog;
export const buildG6SocialSem1BrowserCatalog = catalog.buildBrowserCatalog;

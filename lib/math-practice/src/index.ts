/**
 * Shared concrete math practice.
 *
 * Worksheet / quiz / homework / activity all pull from here so math never
 * falls back to abstract meta-prompts ("اشرح كيف يساعدك فهم X"). Since
 * 2026-09-13 the evaluation generator on the API server draws from it too,
 * which is why this lives in `lib/` rather than in the mobile bundle: an exam
 * that marks itself needs items whose answers are known, and the only two
 * sources of those are a real model and this bank.
 *
 * **It deliberately does not import the knowledge base.** `knowledgeBase.ts` is
 * 3,000 lines pulling in every curriculum catalog the app ships, and it only
 * ever supplied two things here: a lesson type, and one `getBookForLesson`
 * lookup for a subject id. Both are now parameters — the caller already knows
 * its own subject, and dragging the catalogs onto the server to re-derive it
 * would have made this package unusable there.
 */

/**
 * The fields this bank actually reads off a lesson.
 *
 * Structural on purpose: the mobile app's `KBLesson` satisfies it without
...
/** Retained name so the mobile call sites read unchanged after the move. */
export type KBLesson = PracticeLesson;
...
export function takeConcreteMath(
  ...
  points: number,
  /**
   * Which items this pass has already spent. Defaults to the module-level set,
   * which is what the app wants: one user, one generation at a time.
   *
   * The API server must pass its own. `usedIds` is module state, and on a
   * long-running Express process that means one bank shared by every teacher
   * on the box — it would drain until later requests got nothing, and two
   * concurrent generations would clear each other's set mid-paper. A set per
